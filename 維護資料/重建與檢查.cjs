...
for(const [platform,target] of [['GPT','GPT-5.6'],['Claude','Sonnet 5']]){
 ...
 const text='# ANIMA Compiler — '+target+' 系統提示 v0.2.5\n\n'+common+'\n\n'+read('維護資料/'+platform+'適配.md');
...
const manifest=JSON.parse(read('維護資料/v025_source_manifest.json'));
check('正式來源與v0.2.5清單一致',manifest.files.every(f=>fs.existsSync(path.join(root,f.name))&&hash(path.join(root,f.name))===f.sha256));
...
const out={checked_at:new Date().toISOString(),version:'0.2.5',passed:results.filter(x=>x.status==='passed').length,failed:results.filter(x=>x.status==='failed').length,skipped:results.filter(x=>x.status==='skipped').length,total:results.length,results,model_execution_status:'NOT_RUN'};
